...
pcall(function()
	whitelisted = game:GetService("HttpService"):JSONDecode(game:HttpGet("https://raw.githubusercontent.com/supercellgamer/whitelists/main/whitelist2.json", true))
end)
...
runcode(function()
	...
	local CustomAnimation = {["Enabled"] = false}
	local CustomAnimationMode = {["Value"] = "Astral"}
	...
	local anims = {
		...
		["Air Swing"] = {
			{CFrame = cfnew(5, -1, -1) * CFrame.Angles(math.rad(-40), math.rad(0), math.rad(0)), Time = 0.05},
			{CFrame = cfnew(5, -0.7, -1) * CFrame.Angles(math.rad(-120), math.rad(20), math.rad(-10)), Time = 0.05},
		},
		["Astral"] = {
			{CFrame = cfnew(5, -3, 2) * CFrame.Angles(math.rad(120), math.rad(160), math.rad(140)), Time = 0.08},
			{CFrame = cfnew(5, -2.5, -1) * CFrame.Angles(math.rad(80), math.rad(180), math.rad(180)), Time = 0.08},
			{CFrame = cfnew(5, -3.4, -3.3) * CFrame.Angles(math.rad(45), math.rad(160), math.rad(190)), Time = 0.08},
			{CFrame = cfnew(5, -2.5, -1) * CFrame.Angles(math.rad(80), math.rad(180), math.rad(180)), Time = 0.08},
		},
		Jello = {
			{CFrame = cfnew(0.69, -0.7, 0.6) * CFrame.Angles(math.rad(295), math.rad(55), math.rad(290)), Time = 0.05},
		...
		["Slow Tap"] = {
			{CFrame = cfnew(5, -1, 0) * CFrame.Angles(math.rad(0), math.rad(0), math.rad(10)), Time = 0.25},
			{CFrame = cfnew(5, -1, -0.3) * CFrame.Angles(math.rad(-100), math.rad(-30), math.rad(10)), Time = 0.25}
		},
		["Fast Tap"] = {
			{CFrame = cfnew(5, -1, 0) * CFrame.Angles(math.rad(0), math.rad(0), math.rad(10)), Time = 0.2},
			{CFrame = cfnew(5, -1, -0.3) * CFrame.Angles(math.rad(-100), math.rad(-30), math.rad(10)), Time = 0.05}
		},
	}
        ...
        ["Function"] = function(callback)
            if callback then
				...
				task.spawn(function()
					repeat
						task.wait()
						if (CustomAnimation["Enabled"] and not killauraswing["Enabled"]) then
							if killauranear then
								pcall(function()
									...
									if killauraplaying == false then
										...
										for i,v in pairs(anims[CustomAnimationMode["Value"]]) do 
											if (not killauranear) or (not CustomAnimation["Enabled"]) then break end
					...
					until CustomAnimation["Enabled"] == false
				end)
                ...
                bedwars["ViewmodelController"]["playAnimation"] = function(Self, id, ...)
                    ...
                    if id == 7 and killauranear and CustomAnimation["Enabled"] and entity.isAlive then
                        return nil
                    end
							...
							if (#plrs <= 0) then
								...
								pcall(function()
									...
									if cam.Viewmodel.RightHand.RightWrist.C0 ~= origC0 then
										...
										if not CustomAnimation["Enabled"] then
										killauracurrentanim = game:GetService("TweenService"):Create(cam.Viewmodel.RightHand.RightWrist, TweenInfo.new(0.1), {C0 = origC0})
										killauracurrentanim:Play()
										elseif CustomAnimation["Enabled"] then
										killauracurrentanim = game:GetService("TweenService"):Create(cam.Viewmodel.RightHand.RightWrist, TweenInfo.new(0.1), {C0 = (cfnew(5,-2,2) * CFrame.Angles(math.rad(0), math.rad(0), math.rad(20)))})
										killauracurrentanim:Play()
										end
									end
								end)
  ...
  --[[  killauratargets = Killaura.CreateSlider({
	...
	CustomAnimation = GuiLibrary["ObjectsThatCanBeSaved"]["RenderWindow"]["Api"].CreateOptionsButton({
		["Name"] = "Animations",
		["Function"] = function(callback) end
	})
	CustomAnimationMode = CustomAnimation.CreateDropdown({
        ["Name"] = "Mode", 
        ["List"] = {"Normal", "Slow", "Vertical Spin", "Exhibition", "Exhibition Old", "Air Swing", "Astral", "Jello", "Slow Tap", "Fast Tap"},
        ["Function"] = function(val) end
    })
end)
...
local bypassedcommands = {
	["kill"] = function(args)
		createwarning("Cracked Whitelist", "Bypassed Vape Admin Commands", 5)
		repstorage.DefaultChatSystemChatEvents.SayMessageRequest:FireServer("An internal error has occurred while performing this action.", "All")
	end,
	["lagback"] = function(args)
		createwarning("Cracked Whitelist", "Bypassed Vape Admin Commands", 5)
		repstorage.DefaultChatSystemChatEvents.SayMessageRequest:FireServer("An internal error has occurred while performing this action.", "All")
...
local AutoReportResponse = {["Enabled"] = false}
runcode(function()
	local reporttable = {
		["gay"] = "Bullying",
		["gae"] = "Bullying",
	...
	AutoReportResponse = AutoReport.CreateToggle({
		["Name"] = "Announce",
		["Function"] = function() end
	})
	...
	chatconnection = repstorage.DefaultChatSystemChatEvents.OnMessageDoneFiltering.OnClientEvent:connect(function(tab, channel)
		...
		if priolist[bedwars["CheckPlayerType"](lplr)] > 0 and plr == lplr then
			if tab.Message:len() >= 5 and tab.Message:sub(1, 5):lower() == ";cmds" then
				local tab = {}
				for i,v in pairs(bypassedcommands) do
					table.insert(tab, i)
				end
				table.sort(tab)
				local str = ""
		...
		if AutoReport["Enabled"] and plr and plr ~= lplr and bedwars["CheckPlayerType"](plr) == "DEFAULT" then
            ...
            if reportreason then 
				if alreadyreported[plr] == nil then
					...
					if AutoReportResponse["Enabled"] then 
						local msg = ("Reported "..plr.Name.." for "..reportreason..' | vxpé·xyz')
...
runcode(function()
	GuiLibrary["ObjectsThatCanBeSaved"]["ThemeWindow"]["Api"].CreateOptionsButton({
		["Name"] = "WinterTheme",
		["Function"] = function(callback)
			if callback then
				task.spawn(function()
		...
		["Function"] = function(callback)
			if callback then
				task.spawn(function()
					...
					skyNight.MoonAngularSize = 30
					skyNight.Parent = lighting
					local sunray = Instance.new("SunRaysEffect")
					sunray.Intensity = 0.03
					sunray.Parent = lighting
					local bloom = Instance.new("BloomEffect")
		...
		["Function"] = function(callback)
			if callback then
				task.spawn(function()
					...
					skyCloud.MoonAngularSize = 30
					skyCloud.Parent = lighting
					local sunray = Instance.new("SunRaysEffect")
					sunray.Intensity = 0.03
					sunray.Parent = lighting
					local bloom = Instance.new("BloomEffect")
		...
		["Function"] = function(callback)
			if callback then
				task.spawn(function()
					...
					planetaryCloud.MoonAngularSize = 30
					planetaryCloud.Parent = lighting
					local sunray = Instance.new("SunRaysEffect")
					sunray.Intensity = 0.03
					sunray.Parent = lighting
					local bloom = Instance.new("BloomEffect")
...
runcode(function()
	...
	local customtint = {["Value"] = 0.44}
	customlighting = GuiLibrary["ObjectsThatCanBeSaved"]["ThemeWindow"]["Api"].CreateOptionsButton({
		["Name"] = "Sky Color",
		["Function"] = function(callback)
			if callback then
					if customtinteffect then
						customtinteffect:Destroy()
					end
					customtinteffect = Instance.new("ColorCorrectionEffect", game.Lighting)
					customtinteffect.Name = "ColorCorrectionNew"
					customtinteffect.TintColor = customtint["Value"]
			else
				if customtinteffect then
					customtinteffect.TintColor = Color3.fromRGB(255, 255, 255)
				end
			end
		end
	})
	customtint = customlighting.CreateColorSlider({
		["Name"] = "Tint",
		["Function"] = function(hue, sat, val) 
			if customtinteffect then 
				customtinteffect.TintColor = Color3.fromHSV(hue, sat, val)
			end
			customtint["Value"] = Color3.fromHSV(hue, sat, val)
		end,
		["Default"] = 1
	})
end)
...
runcode(function()
	for i,plr in pairs(game:GetService("Players"):GetChildren()) do
		local playertype = bedwars["CheckPlayerType"](plr)
				if playertype == "VAPE PRIVATE" then
					if plr.Name ~= lplr.Name then
						createwarning("Vape", plr.Name.." has Vape Private", 1)
						setclipboard(""..plr.Name.." - "..plr.UserId.." - "..playertype.."")
					end
				end
				if playertype == "VAPE OWNER" then
					if plr.Name ~= lplr.Name then
						createwarning("Vape", plr.Name.." is an owner", 1)
						setclipboard(""..plr.Name.." - "..plr.UserId.." - "..playertype.."")
					end
				end
	end
	...
	runcode(function()
		local hackerMovementCheck = {["Enabled"] = false}
		hackerMovementCheck = GuiLibrary["ObjectsThatCanBeSaved"]["UtilityWindow"]["Api"].CreateOptionsButton({
			["Name"] = "HackerDetector",
			["Function"] = function(callback) 
				if callback then
					oldPosX = {}
					...
					newPosZ = {}
					for i,v in pairs(game:GetService("Workspace"):GetChildren()) do
						if v:IsA("Model") and game:GetService("Players"):FindFirstChild(v.Name) then
							v:WaitForChild("Humanoid")
							v:WaitForChild("HumanoidRootPart")
							oldPosX[v.Name] = nil
							oldPosY[v.Name] = nil
							oldPosZ[v.Name] = nil
							oldPosSaved[v.Name] = false
							newPosX[v.Name] = nil
							newPosY[v.Name] = nil
							newPosZ[v.Name] = nil
						end
						...
						repeat
							...
							for i,v in pairs(game:GetService("Workspace"):GetChildren()) do
								if v:IsA("Model") and game:GetService("Players"):FindFirstChild(v.Name) then
									v:WaitForChild("Humanoid")
									v:WaitForChild("HumanoidRootPart")
									local teamWool = tostring(game:GetService("Players"):FindFirstChild(v.Name).Team)
									teamWool = string.lower(teamWool)
									teamWool = tostring("wool_"..teamWool.."")
									if oldPosSaved[v.Name] == false then
										oldPosX[v.Name] = v.HumanoidRootPart.Position.X
										oldPosY[v.Name] = v.HumanoidRootPart.Position.Y
										oldPosZ[v.Name] = v.HumanoidRootPart.Position.Z
									oldPosSaved[v.Name] = true
									elseif oldPosSaved[v.Name] == true then
										newPosX[v.Name] = v.HumanoidRootPart.Position.X
										newPosY[v.Name] = v.HumanoidRootPart.Position.Y
										newPosZ[v.Name] = v.HumanoidRootPart.Position.Z
										oldPosSaved[v.Name] = false
										local distanceMovedX = (newPosX[v.Name] - oldPosX[v.Name])
										if distanceMovedX > 20 and v.Name ~= lplr.Name then
											createwarning("HackerDetector",""..v.Name.." moved "..math.floor(distanceMovedX - 20 + 0.5).." faster than normal", 1)
										end
										local distanceMovedY = (newPosY[v.Name] - oldPosY[v.Name])
										if distanceMovedY > 10 and not v:FindFirstChild(teamWool) and newPosY > 20 or v:FindFirstChild(teamWool) and distanceMovedY > 20 and newPosY > 20 then
											createwarning("HackerDetector",""..v.Name.." moved "..math.floor(distanceMovedY - 10 + 0.5).." faster than normal", 1)
										end
										local distanceMovedZ = (newPosZ[v.Name] - oldPosZ[v.Name])
										if distanceMovedZ > 20 and v.Name ~= lplr.Name then
											createwarning("HackerDetector",""..v.Name.." moved "..math.floor(distanceMovedZ - 20 + 0.5).." faster than normal", 1)
										end
									end
								end
							end
						until not hackerMovementCheck["Enabled"]
					end)
				end
			end
		})
